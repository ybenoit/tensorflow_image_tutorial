...
from tensorflow_image_tutorial.operation import InputOp, InferenceOpWithTwoConv, LossOp, TrainingOp, EvaluationOp
...
tf.app.flags.DEFINE_string('train_dir', '/tmp/not_mnist/not_mnist_train/two_conv',
                           """Directory where to write event logs and checkpoint.""")
tf.app.flags.DEFINE_string('summaries_dir', '/tmp/not_mnist/not_mnist_logs/two_conv', """Summaries directory""")
...
def main():
    ...
    with tf.Graph().as_default():
        ...
        softmax, logits = InferenceOpWithTwoConv(
            num_pixels=Constant.NUM_PIXELS,
            num_classes=Constant.NUM_CLASSES,
            num_neurons_in_first_dense_hidden_layer=64,
            num_channels_in_first_conv_layer=16,
            num_channels_in_second_conv_layer=32
        ).add_ops(x_reshaped)
